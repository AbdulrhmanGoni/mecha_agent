...
export default function activateApiKeyTests({ db }: { db: PostgresClient }) {
    const endpoint = "/api/api-keys/activate";
    ...
    describe(`Testing 'PATCH ${endpoint}' route`, () => {
        ...
        it("Should fail to activate anything because the user isn't existant", async () => {
            ...
            expect(res.error).toBe(apiKeysResponseMessages.failedKeyActivation);
        });
        ...
        it("Should succeed to activate all api keys of the user", async () => {
            ...
            expect(res.result).toBe(apiKeysResponseMessages.successfulKeyActivation);
