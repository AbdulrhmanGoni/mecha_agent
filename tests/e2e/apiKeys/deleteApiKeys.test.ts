...
export default function deleteApiKeyTests({ db }: { db: PostgresClient }) {
    ...
    describe(`Testing 'DELETE ${endpoint}' route`, () => {
        ...
        it("Should fail to delete anything because the user isn't existant", async () => {
            ...
            expect(res.error).toBe(apiKeysResponseMessages.failedKeyDeletion);
        });
        ...
        it("Should succeed to delete all api keys of the user", async () => {
            ...
            expect(res.result).toBe(apiKeysResponseMessages.successfulKeyDeletion);

            const { rowCount } = await db.queryObject(
                `SELECT id FROM api_keys WHERE user_email = '${testingUserCredentials.email}'`
            )
